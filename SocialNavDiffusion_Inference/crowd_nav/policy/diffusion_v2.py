...
class DiffusionV2(Policy):
    ...
    def configure(self, config):
        ...
        self.start_dim = config.getint('diffusion_v2', 'start_dim')
        self.goal_dim = config.getint('diffusion_v2', 'goal_dim')
        self.horizon = config.getint('diffusion_v2', 'horizon')
        self.k_max = config.getint('diffusion_v2', 'k_max')
        self.kinematics = config.get('action_space', 'kinematics')
        self.multiagent_training = config.getboolean('diffusion_v2', 'multiagent_training')

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.device = device

        # Load normalization stats
        norm_file = config.get('diffusion_v2', 'norm_file')
        ...
        # Load checkpoint
        ckpt_path = config.get('diffusion_v2', 'ckpt_path')
