...
class DiffusionV2WF(Policy):
    def __init__(self):
        ...
        self.name = "DiffusionV2WF"

    def configure(self, config):
        import torch
        from crowd_nav.policy.run_socialrobot_v2 import ConditionalTemporalNet, GaussianDiffusionSimple
        import numpy as np

        self.start_dim = config.getint('diffusion_v2wf', 'start_dim')
        self.goal_dim = config.getint('diffusion_v2wf', 'goal_dim')
        self.horizon = config.getint('diffusion_v2wf', 'horizon')
        self.k_max = config.getint('diffusion_v2wf', 'k_max')
        self.kinematics = config.get('action_space', 'kinematics')
        self.multiagent_training = config.getboolean('diffusion_v2wf', 'multiagent_training')

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.device = device

        # Load normalization stats
        norm_file = config.get('diffusion_v2wf', 'norm_file')
        self.norm = np.load(norm_file, allow_pickle=True).item()
        
        # Construct model + diffusion
        self.model = ConditionalTemporalNet(traj_dim=2, cond_dim=256).to(device)
        self.diffusion = GaussianDiffusionSimple(device=device)

        # Load checkpoint
        ckpt_path = config.get('diffusion_v2wf', 'ckpt_path')
