...
class SceneTokenEmbedder(nn.Module):
    def __init__(self, start_dim, goal_dim, obs_dim, token_dim, k_max,
        ...
        # Null token now sized to include map tokens too — used for CFG dropout
        self.null_token = nn.Parameter(
            torch.randn(1, 2 + k_max + n_map_tokens, token_dim) * 0.02
        )
    ...
    def forward(self, start, goal, obstacles, obs_mask=None,
        ...
        B = start.shape[0]
        start_tok = self.start_proj(start).unsqueeze(1) + self.type_embed[0]
        goal_tok  = self.goal_proj(goal).unsqueeze(1)   + self.type_embed[1]
        obs_tok   = self.obs_proj(obstacles)            + self.type_embed[2]
        tokens = torch.cat([start_tok, goal_tok, obs_tok], dim=1)  # [B, 2+K, D]

        # Build base validity mask: start + goal always valid, obstacles per mask
        if obs_mask is not None:
            valid = torch.cat(
                [torch.ones(B, 2, device=tokens.device), obs_mask], dim=1
            )
        else:
            valid = torch.ones(B, tokens.shape[1], device=tokens.device)
        ...
        if self.use_map:
            ...
            map_tokens = self.map_encoder(occ_map, has_map)  # [B, n_map, D]
            tokens = torch.cat([tokens, map_tokens], dim=1)
            map_valid = torch.ones(B, self.n_map_tokens, device=tokens.device)
            valid = torch.cat([valid, map_valid], dim=1)

        key_padding_mask = ~valid.bool()  # True = ignore
        tokens = self.scene_encoder(tokens, src_key_padding_mask=key_padding_mask)
        return self.final_norm(tokens)


def build_attn_mask(obs_mask, n_map_tokens=0):
    """
    obs_mask: [B, K]
    n_map_tokens: int — number of map tokens appended after obstacle tokens
    """
    B, K = obs_mask.shape
...
class DiffusionConditionalUNet1DCFG(Policy):
    ...
    def configure(self, config):
        ...
        if "ema_model_state_dict" in ckpt:
            ...
            self.token_embedder.load_state_dict(ckpt["ema_token_embedder_state_dict"])
        else:
    ...
    def _rasterize_ego_map(self, state):
        ...
        cols = np.round(x_ego / res + W / 2 - 0.5).astype(np.int32)
        rows = np.round(y_ego / res + H / 2 - 0.5).astype(np.int32)
    ...
    def predict(self, state):
        ...
        with torch.no_grad():
            tokens = self.token_embedder(start, goal, obstacles, obs_mask, occ_map=occ_map_t, has_map=has_map_t)
            n_map_tokens = self.token_embedder.n_map_tokens if self.use_map else 0
            attn_mask = build_attn_mask(obs_mask, n_map_tokens=n_map_tokens)

            null_tokens      = self.token_embedder.null_token.expand(1, -1, -1)
            attn_mask_uncond = torch.ones(1, tokens.shape[1],
                                        device=self.device, dtype=attn_mask.dtype)

            tokens_k      = tokens.expand(K, -1, -1)
            attn_mask_k   = attn_mask.expand(K, -1)
            null_tokens_k = null_tokens.expand(K, -1, -1)
            attn_mask_unc = attn_mask_uncond.expand(K, -1)
