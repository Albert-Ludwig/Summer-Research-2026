...
class DiffusionV1(Policy):
    ...
    def configure(self, config):
        ...
        self.start_dim = config.getint('diffusion_v1', 'start_dim')
        self.goal_dim = config.getint('diffusion_v1', 'goal_dim')
        self.horizon = config.getint('diffusion_v1', 'horizon')
        self.k_max = config.getint('diffusion_v1', 'k_max')
        self.kinematics = config.get('action_space', 'kinematics')
        self.multiagent_training = config.getboolean('diffusion_v1', 'multiagent_training')
        ...
        norm_file = config.get('diffusion_v1', 'norm_file')
        ...
        # Load checkpoint
        ckpt_path = config.get('diffusion_v1', 'ckpt_path')
    ...
    def predict(self, state):
        ...
        with torch.no_grad():
            traj, traj_snapshots = self.diffusion.p_sample_loop(
                self.model,
                shape,
                cond,
                self.device,
        ...
        dx, dy = traj_world[int(np.floor(dt))-1] * (1 - interp) + traj_world[int(np.floor(dt))] * interp
        print(dx,dy)
