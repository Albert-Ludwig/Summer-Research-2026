...
class CrossAttention(nn.Module):
    def __init__(self, d_model, cond_dim, num_heads=4):
        super().__init__()
        self.query_proj = nn.Linear(d_model, d_model)
        self.key_proj   = nn.Linear(cond_dim, d_model)
        self.value_proj = nn.Linear(cond_dim, d_model)
    ...
    def forward(self, x, cond, key_padding_mask=None):
        """
        x:    (B, T, d_model)     → queries
        cond: (B, N_cond, cond_dim) → keys/values
        """
        q = self.query_proj(x)
...
class ConditionalTemporalNet(nn.Module):
    def __init__(self, traj_dim, cond_dim, hidden_dim=256, num_layers=5, kernel_size=3):
        ...
        self.start_encoder = nn.Linear(4, cond_dim)
        self.goal_encoder  = nn.Linear(2, cond_dim)
        self.obs_encoder   = nn.Linear(4, cond_dim)
        ...
        # Project trajectory + time embedding
        self.input_proj = nn.Linear(traj_dim + hidden_dim, hidden_dim)
        ...
        # Temporal conv stack (all hidden_dim)
        layers = []
    ...
    def forward(self, x_noisy, t, start, goal, obstacles, obs_mask):
        B, T, _ = x_noisy.shape
        ...
        # ---- conditioning tokens ----
        start_tok = self.start_encoder(start).unsqueeze(1)
        goal_tok  = self.goal_encoder(goal).unsqueeze(1)

        if obstacles is not None:
            obs_tok = self.obs_encoder(obstacles)

            cond_tokens = torch.cat([start_tok, goal_tok, obs_tok], dim=1)

            start_goal_mask = torch.zeros(B, 2, dtype=torch.bool, device=start.device)
            obs_padding_mask = (obs_mask == 0)

            key_padding_mask = torch.cat(
                [start_goal_mask, obs_padding_mask],
                dim=1
            )
        else:
            cond_tokens = torch.cat([start_tok, goal_tok], dim=1)
...
class GaussianDiffusionSimple:
    ...
    def p_sample_loop(self, model, shape, start, goal, obstacles, obs_mask, device, log_steps=5):
        B, T, D = shape
        ...
        for batch in dataloader:
            ...
            if step % save_every == 0:
                ...
                with torch.no_grad():
                    ...
                    sample, traj_snapshots = diffusion.p_sample_loop(model, (1, T, D), start[:1], goal[:1], obstacles[:1], obs_mask[:1], device, log_steps=5)
                    ...
                    traj_mse = ((sample_denorm - gt_denorm) ** 2).mean().item()
                    wandb.log( #Scaled version of ADE, same purpose
                        {
                            "sample/MSE": traj_mse,
                        },
                        step=step,
...
def main():
    ...
    cond_dim = args.hidden_dim  # start + goal + obstacles flat + mask
    ...
    dummy_start = torch.randn(B, 4, device=device)     # start_dim = 4
    dummy_goal  = torch.randn(B, 2, device=device)     # goal_dim = 2

    dummy_obstacles = torch.randn(B, K, 4, device=device)
    dummy_obs_mask  = torch.ones(B, K, device=device)  # IMPORTANT: not all zeros
