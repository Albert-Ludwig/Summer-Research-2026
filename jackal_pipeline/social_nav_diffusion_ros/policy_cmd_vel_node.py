...
class PolicyCmdVelNode(Node):
    ...
    def log_startup_audit_state(self):
        ...
        if not math.isnan(policy_dt) and abs(policy_dt - self.diffusion_inference_period) > 1e-6:
            self.get_logger().warn(
                ...
                "Current wrapper uses zero-order hold of the selected command; future work should sample "
                "the projected v(t), omega(t) trajectory at cmd_publish_period_sec when a clean public API is available."
            )
    ...
    # The current high-rate publisher intentionally uses zero-order hold of the latest
    # selected command. Future work should sample the policy/projection v(t), omega(t)
    # trajectory at cmd_publish_period_sec if the policy exposes a stable public API.
    ...
    def policy_debug_callback(self):
        ...
        fields = {
            ...
            "timing_warning": "" if (math.isnan(self.get_policy_time_step()) or abs(self.get_policy_time_step() - self.diffusion_inference_period) <= 1e-6) else "policy_time_step != diffusion_inference_period_sec; wrapper uses zero-order hold",
